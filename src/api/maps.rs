...
async fn get_vn_scores(Path(map_md5): Path<String>, q: Query<QueryFormat>) -> Result<HttpResponse, SeverError> {
  Ok(HttpResponse::Ok().json(Score::map_vn_scores(map_md5, q.into_inner())?))
}

#[get("/{map_md5}/rx_scores")]
async fn get_rx_scores(Path(map_md5): Path<String>, q: Query<QueryFormat>) -> Result<HttpResponse, SeverError> {
  Ok(HttpResponse::Ok().json(Score::map_rx_scores(map_md5, q.into_inner())?))
}

#[get("/{map_md5}/ap_scores")]
async fn get_ap_scores(Path(map_md5): Path<String>, q: Query<QueryFormat>) -> Result<HttpResponse, SeverError> {
